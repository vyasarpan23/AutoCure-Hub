...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (sidebarLogoutBtn) sidebarLogoutBtn.addEventListener("click", logoutFunction);
  ...
  function addEventListeners() {
      ...
      document.querySelectorAll(".delete-btn").forEach(btn => {
          btn.addEventListener("click", async (event) => {
              ...
              try {
                  await fetch(`http://localhost:8080/gallery/${imageId}`, { method: "DELETE" });
      ...
      document.querySelectorAll(".replace-btn").forEach(btn => {
          btn.addEventListener("click", (event) => {
              const imageId = event.target.dataset.id;
              const fileInput = document.querySelector(`.replace-input[data-id='${imageId}']`);
              fileInput.click();
          });
      });
      ...
      document.querySelectorAll(".replace-input").forEach(input => {
          input.addEventListener("change", async (event) => {
              ...
              try {
                  const response = await fetch(`http://localhost:8080/gallery/${imageId}`, {
                      method: "PUT",
                      body: formData
                  });
                  const updatedImage = await response.json();
                  allImages = allImages.map(img => img.id === imageId ? updatedImage : img);
                  displayImages(showingAll);
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  function displayEmployees(showAll) {
      ...
      employeesToShow.forEach(employee => {
          ...
          row.innerHTML = `
              ...
              <td class="py-2 px-4 text-right ${employee.status === 'Active' ? 'text-green-600' : 'text-yellow-600'} ">${employee.status}</td>
              <td class="py-2 px-4 text-right">
                  
                   <button class="details-btn bg-blue-600 text-white p-2 rounded hover:bg-blue-700" data-id="${employee.id}">Details</button>
                  <button class="remove-btn bg-red-600 text-white p-2 rounded hover:bg-red-700" data-id="${employee.id}">Remove</button>
              </td>
          `;
  ...
  function addEventListeners() {
      ...
      document.querySelectorAll(".remove-btn").forEach(btn => {
          btn.addEventListener("click", async (event) => {
            if(confirm("Are you sure you want to remove this employee?")){
              ...
              try {
                  await fetch(`http://localhost:8080/employees/${employeeId}`, { method: "DELETE" });
      ...
      document.querySelectorAll(".details-btn").forEach(btn => {
        ...
        btn.addEventListener("click", async (event) => {
            const employeeId = event.target.dataset.id;
            try {
                const response = await fetch(`http://localhost:8080/employees/details/${employeeId}`);
  ...
  function showEmployeePopup(employee) {
    ...
    popup.classList.add("fixed", "top-0", "left-0", "w-full", "h-full", "bg-gray-800", "bg-opacity-50", "flex", "justify-center", "items-center");
    popup.innerHTML = `
        <div class="bg-white p-6 rounded shadow-lg w-96">
            ...
            <p><strong>Status:</strong> <span class="${employee.status === 'Active' ? 'text-green-600' : 'text-yellow-600'}">${employee.status}</span></p>
            <p><strong>Security Key:</strong> ${employee.security_key}</p>
            <button class="close-popup bg-red-600 text-white px-4 py-2 mt-4 rounded">Close</button>
        </div>
    `;
      ...
      tasks.forEach(task => {
          ...
          row.innerHTML = `
            <td class="py-2 px-4">${task.owner_name}</td>
              ...
              <td class="py-2 px-4">
                  <select class="employee-select p-2 border rounded" data-task-id="${task.id}">
              ...
              <td class="py-2 px-4 ${task.employee_id ? "text-green-600" : "text-yellow-600"}">
                  ${task.employee_id ? "Assigned" : "To Be Assigned"}
              </td>
              <td class="py-2 px-4 ${task.status === "pending" ? "text-red-600" : "text-yellow-600"}">
                    ${task.status === "completed" ? 
                        `<button class="complete-btn bg-green-600 text-white p-2 rounded hover:bg-green-700" data-id="${task.id}">
                            Complete
                        </button>` 
                        : task.status}
                </td>
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderNotifications() {
      ...
      let visibleNotifications = showAll ? notifications : notifications.slice(-3);

      visibleNotifications.forEach((notif) => {

        const formattedDate = new Date(notif.booking_date).toLocaleDateString("en-GB", {
            day: "2-digit",
            month: "2-digit",
            year: "numeric"
        });
...
function removeNotification(bookingId) {
  notifications = notifications.filter(notif => notif.booking_id !== bookingId);
  renderNotifications();
  ...
  function openTimeChangePopup(bookingId) {
      ...
      if (newDate && newTime) {
          fetch(`http://localhost:8080/notifications/change-time/${bookingId}`, {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ booking_date: newDate, booking_time: newTime },  managerId),
